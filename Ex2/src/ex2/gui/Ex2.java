 ...
 * A general explanation-The Ex2 project is built from classes.
 * Each class contains a shape and the functions it implements for example (area, perimeter, and else...).
 * For each shape, I built the relevant functions to be able to run and perform actions without limitation.
 * In addition, there are other classes-
 * Shapecomp- performs a comparison between different attributes between the forms.
 * The shapes can be sorted according to the comparison in the function "sort" which is in the next class-
 * Shapecollection- in this class, there are functions that allow extensive operations to be performed on various shapes.
 * "sort" function - allows you to sort shapes by - area, perimeter, tag, and string(and their opposites)
 * and arrange them in ascending order.
 * "save" function - allows you to save the shapes as a file on the computer.
 * "load" function - allows to open the saved file with the saved shapes without changes.
 * In addition, there are additional functions that allow simpler operations.
 * Guishape- is a class that is important for drawing shapes.
 * Because in this class we define for each shape its attributes (such as color, size, and else.)
 * In addition, I built a helper function for the load that receives a string (of a shape) and divides it into parts,
 * each part being an attribute of the shape. It then builds a shape from all the attributes
 * and passes it to the load function listed above.
    ...
    public void mouseClicked(Point_2D p) {
        ...
        if (_mode.equals("Triangle")) {
            ...
            if (_pp.getAllPoints().length == 3) {
                ...
                GUI_Shape s = new GUIShape(tt, _fill, _color, 3);
                _shapes.add(s);
                _pp = null;
                _p1 = null;
                _gs = null;
